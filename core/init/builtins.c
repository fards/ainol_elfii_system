...
int do_mount(int nargs, char **args)
{
    ...
    for (n = 4; n < nargs; n++) {
        ...
        if (!mount_flags[i].name) {
            if (!strcmp(args[n], "wait"))
                wait = 1;
 	    else if(!strcmp(args[n],"no_data_secure"))
		no_data_secure=1;
    ...
    if (!strcmp(target, DATA_MNT_POINT)) {
        property_set("ro.userdata.fs_type", system);
    }
    ...
    if (!strncmp(source, "mtd@", 4)) {
        ...
        if (mount(tmp, target, system, flags, options) < 0 ) {
             ...
             * encrypted /data.
             */
            if (!strcmp(target, DATA_MNT_POINT) && !partition_wiped(source)&& no_data_secure==0) {
...
exit_success:
    ...
    if (!strcmp(target, DATA_MNT_POINT)) {
        ...
        property_set("ro.userdata.state", "mounted");
