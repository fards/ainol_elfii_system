    ...
    if (!hit) {
        ...
        for (iter = mUms->begin(); iter != mUms->end(); ++iter) {
            if ((*iter)->isInUmsDevPath(devpath)) {
...
#ifdef NETLINK_DEBUG
                    SLOGD("Device '%s' is in by ums path %s\n", devpath, (*iter)->getUmsDevPath());
#endif
				...
				{
					SLOGD("Device '%s' has %d partitions,will mount by partition uevent\n", devpath,DiskNumParts);		
				}
                	} else {
                    DirectVolume *dv = new DirectVolume(this, devname, mount_point, -1);
                    dv->addPath(devpath);
...
int VolumeManager::mountAsec(const char *id, const char *key, int ownerUid) {
    ...
    sync();
remount:
    //SLOGE("ASEC domount(%s,%s)-------%s", dmDevice,mountPoint,asecFileName);
